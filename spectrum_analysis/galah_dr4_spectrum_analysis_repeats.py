...
spectrum['ruwe_gaia_dr3'] = float(init_values_table['ruwe'][np.where(init_values_table['tmass_id'] == spectrum['tmass_id'])[0][0]])
...
plt.savefig(file_directory+str(spectrum['sobject_id'])+'_coadd_rv_comparison.png',bbox_inches='tight')
...
for repeat_index, repeat_sobject_id in enumerate(spectrum['sobject_ids'][1:]):
    ...
    for ccd in [1,2,3,4]:
        ...
        if ccd in spectrum['available_ccds']:
            ...
            spectrum['counts_unc_ccd'+str(ccd)] += np.interp(spectrum['wave_ccd'+str(ccd)], wave_shifted_next, single_spectrum['counts_unc_ccd'+str(ccd)], left = 0, right = 0)
...
def adjust_rv(current_rv, wave_input_for_rv, data_input_for_rv, sigma2_input_for_rv, model_input_for_rv):
    ...
    plt.savefig(file_directory+str(spectrum['sobject_id'])+'_coadd_fit_rv.png',overwrite=True,bbox_inches='tight')
...
fig.savefig(file_directory+str(spectrum['sobject_id'])+'_coadd_fit_comparison.pdf',overwrite=True,bbox_inches='tight')
...
save_spectrum.write(file_directory+str(spectrum['sobject_id'])+'_coadd_fit_spectrum.fits',overwrite=True)
...
# Save covariances
np.savez(
    file_directory+str(spectrum['sobject_id'])+'_coadd_fit_covariances.npz',
...
np.savetxt(file_directory+str(spectrum['sobject_id'])+'_coadd_sobject_ids.txt',spectrum['sobject_ids'],fmt='%s')
...
# And save!
output.write(file_directory+str(spectrum['sobject_id'])+'_coadd_fit_results.fits',overwrite=True)
